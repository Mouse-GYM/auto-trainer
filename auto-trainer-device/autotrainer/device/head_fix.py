...
class HeadFix(Device):
    ...
    def notify_data(self, data: bytes):
        try:
            ...
            for part in parts:
                ...
                sub_parts = part.split("n")

                for jdx in range(len(sub_parts) - 1):
                    if len(sub_parts[jdx]) == 0:
                        continue
                    if sub_parts[jdx].startswith("s"):
                        self._handle_data(sub_parts[jdx])
                    else:
                        self._handle_info(sub_parts[jdx])

                end = sub_parts[len(sub_parts) - 1]
                if end != "":
                    if end.startswith("s"):
                        self._buffer += sub_parts[len(sub_parts) - 1]
                    else:
                        self._handle_info(end)
    ...
    def notify_message(self, kind: int, data: object, context: object = None):
        ...
        if handled:
            logger.debug(f"command {HeadFixMessageKind(kind).name}")
