...
class PosePredict(Process):
    ...
    def __init__(self, network: str, image_queue: CircularImageBuffer, data_queue: Queue, cmd_queue: Queue,
                 msg_queue: Queue):
        super().__init__()
    ...
    def run(self):
        ...
        self._configuration.load_configuration(os.path.join(self._network, "config.yaml"), 1, 0,
                                               self._image_queue.batch_size)

        try:
            self._send_message(AnalysisMessageKind.Initialized, self._configuration.body_parts)
