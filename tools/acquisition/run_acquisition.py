...
def verify_log_location(log_location: str, device_name: str):
    ...
    def int_map_fcn(value: str):
        try:
            ...
            # logger.error("Cannot int(%r): %s", value, err)
            return None
...
def send_commands(mw: MainWindow, exit_event: threading.Event):
    ...
    while not exit_event.isSet():
        ...
        time.sleep(6)
        mw.mouse_seen_action.trigger()
        time.sleep(2)
        mw.pellet_seen_action.trigger()
        time.sleep(tot_dur)
...
def run_acquisition(
    ...
    *,
    ...
    window = MainWindow(app, preferences, configuration, "1.2.0", is_dev)
