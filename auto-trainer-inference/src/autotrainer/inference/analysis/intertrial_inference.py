...
def intertrial_inference(pose_data: ndarray, part_names: List[str], project: ProjectInfo) -> None:
    """
    ...
    df_xyp.to_hdf(project.get_intertrial_pose_path(name=project.camera_1), "df_with_missing", format="table",
                  mode="w")

    right = pose_data[1::2, :]
    indices = range(right.shape[0])
    df_xyp = pandas.DataFrame(right, columns=columns, index=indices)
    df_xyp.to_hdf(project.get_intertrial_pose_path(name=project.camera_2), "df_with_missing", format="table",
                  mode="w")
