...
class TestAutoCloseGate(MockSystemMachine):
    ...
    def _init(self, machine: SystemMachine):
        ...
        cfg = algo.auto_close_gate_on_intersession_config
        cfg.enabled = True
        cfg.session_min_duration = 0
    ...
    def test_auto_close_gate(self, machine, enabled, caplog, sess_duration, sess_min_duration, monkeypatch):
        ...
        orig = machine._consider_close_gate_during_intersession
        ...
        if enabled and sess_duration >= sess_min_duration:
            ...
            if enabled:
                assert "session duration too short, skipping auto-close-gate" in caplog.text
    ...
    def test_with_delay_after_cage_enter(self, machine, caplog, delay):
        ...
        m_timer2 = None
        def consider_again():
            assert m_timer.call_args_list == [mock.call(0.1, machine._consider_close_gate_during_intersession)]
            ...
            with self.patch_timer(f"{machine.__class__.__module__}._consider_close_gate_timer") as m_timer2:
                machine._consider_close_gate_during_intersession()
            assert m_timer.return_value.cancel.call_args_list == [mock.call()]  # ensure prev timer is canceled

        with self.mock_intersession_analysis(concurrent_func=consider_again):
            with self.patch_timer(f"{machine.__class__.__module__}._consider_close_gate_timer") as m_timer:
                self.exit_tunnel()
        assert m_timer2.call_args_list == [mock.call(0.1, machine._consider_close_gate_during_intersession)]
        assert m_timer2.return_value.cancel.call_args_list == [mock.call()]  # given finished intersession

    def test_when_not_anymore_intersession(self, machine, monkeypatch, caplog):
        algo = self.algo
        cfg = algo.auto_close_gate_on_intersession_config
        ...
        assert m_timer.call_args_list == [mock.call(0.1, machine._consider_close_gate_during_intersession)]
        machine._consider_close_gate_during_intersession()
        assert "not anymore intersession, skipping auto-close-gate" in caplog.text
