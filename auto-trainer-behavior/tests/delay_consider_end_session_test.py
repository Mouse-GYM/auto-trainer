...
class TestDelayConsiderEndSession(MockSystemMachine):
    ...
    def test_it_does_not_delay_with_zero_or_negative(self, machine, caplog, delay):
        algo = self.algo
        machine._delay_timer_consider_end_session = delay
    ...
    def test_it_delays_end_session_with_positive_value(self, machine, caplog, delay):
        algo = self.algo
        machine._delay_timer_consider_end_session = delay
        algo.update_pellet_seen(True)
        self.start_session_in_tunnel()
    ...
    def test_it_does_not_consider_if_not_in_session(self, machine, caplog):
        algo = self.algo
        assert not algo.is_in_session
        with self.patch_timer(f"{machine.__class__.__module__}._consider_end_session_timer") as m_timer2:
            machine._consider_end_session(reason=RecordingEndingReason.PELLET_LOADING)
