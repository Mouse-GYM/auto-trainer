...
class LoadCellConfiguration:
    ...
    thrashing_var_weight_threshold: float = 20   # grams
    thrashing_var_minimum_duration: float = 0.15  # seconds
    thrashing_min_ptp_change_count: int = 2
...
def load_cell_configuration_representer(dumper: yaml.SafeDumper, cfg: LoadCellConfiguration) -> yaml.nodes.MappingNode:
    ...
    return dumper.represent_mapping("!LoadCellConfiguration", {
        ...
        "thrashing_var_minimum_delay": cfg.thrashing_var_minimum_duration,
...
class LoadCellMonitor(ObservableObject):
    ...
    def thrashing_var_minimum_delay(self) -> float:
        return self._config.thrashing_var_minimum_duration

    @thrashing_var_minimum_delay.setter
    def thrashing_var_minimum_delay(self, value):
        self._config.thrashing_var_minimum_duration = value
    ...
    def _update_history(self, value, when, index):
        ...
        keep = max(
            ...
            cfg.thrashing_var_minimum_duration,
        )
        while len(hist) > 0:
            h0_when = hist[0][1]
    ...
    def update(self, value: Union[float, numpy.floating], when: float, index: int):
        ...
        if value > cfg.weight_active_threshold:
            ...
            if t_start is None:
                ...
                if when - self._t_start_was_active > cfg.threshold_duration and not self._is_engaged:
                    self._ensure_active()
                if self._cur_ptp_count == 0 or when - self._t_last_ptp_change > cfg.thrashing_var_minimum_duration:
                    ptp_value1 = numpy.ptp([
                        h_val
                        for h_val, h_when, _ in self._values_history
                        if when - h_when < cfg.thrashing_var_minimum_duration
                    ])
                    new_detected1 = bool(
                    ...
                    ptp_value2 = numpy.ptp([v for v, w, _ in self._values_history if when - w < 0.5 * cfg.thrashing_var_minimum_duration])
                        ...
                        if self._cur_ptp_count == 0 or when - self._t_last_ptp_change > cfg.thrashing_var_minimum_duration:
                            ...
                            if self._cur_ptp_count > cfg.thrashing_min_ptp_change_count:
                                self._thrashing_detected = self._on_property_changed(
                                    self.IS_THRASHING_DETECTED_PROPERTY, True, prev_detected)
                    else:
                        self._cur_ptp_count = 0
                        if when - self._t_last_ptp_change > cfg.threshold_duration:
                            self._thrashing_detected = self._on_property_changed(
                                self.IS_THRASHING_DETECTED_PROPERTY, False, prev_detected)
        else:
            if value < cfg.weight_inactive_threshold:
                ...
                duration = cfg.threshold_duration
                ...
                if self._is_engaged and when - self._t_inactive_start > cfg.threshold_duration:
